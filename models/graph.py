...
class GraphNode(BaseModel):
    ...
    is_abstract: bool = Field(
        False,
        description="Whether this is an abstract/synthetic node"
    )
...
class GraphEdge(BaseModel):
    ...
    is_default: bool = Field(
        False,
        description="Whether this is default path"
    )
    ...
    weight: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Edge weight/probability"
    )
...
class ProcessGraph(BaseModel):
    ...
    has_implicit_parallelism: bool = Field(
        default=False,
        description="Whether graph uses implicit parallelism"
    )
    ...
    def validate_structure(self) -> Tuple[bool, List[str]]:
        ...
        if len(starts) > 1:
            ...
            if not has_fork:
                errors.append(f"Multiple disconnected start points found")
...
class GraphMetrics(BaseModel):
    ...
    structural_completeness: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Score indicating graph is complete (0-1)"
    )
    ambiguity_score: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="Score indicating ambiguity in graph (0-1)"
    )
...
class GraphValidationReport(BaseModel):
    ...
    issues: List[str] = Field(
        ...
        description="Structural issues found"
    )
    warnings: List[str] = Field(
        default_factory=list,
        description="Warnings and recommendations"
    )
