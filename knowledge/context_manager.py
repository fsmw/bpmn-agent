...
class ContextSelector:
    ...
    def select_context(
        ...
        domain_terms = self._extract_domain_terminology(
            domain, token_budgets["terminology"]
        )
        ...
        rules_to_select = self._select_validation_rules(
            patterns_to_select, token_budgets["rules"]
        )
        ...
        package.confidence = min(
            1.0,
            (domain_conf + complexity_score + len(patterns) * 0.2) / 3
        )
        package.reasoning = self._generate_reasoning(
            domain, complexity, patterns, domain_conf
        )
    ...
    def _get_token_budgets(optimization_level: str, max_tokens: int) -> Dict[str, int]:
        ...
        return {
            component: int(max_tokens * ratio)
    ...
    @staticmethod
    def _extract_domain_terminology(
        domain: DomainType,
        token_budget: int
    ) -> Dict[str, List[str]]:
        """
    ...
    @staticmethod
    def _select_validation_rules(
        patterns: List[BPMNPattern],
        token_budget: int
    ) -> List[str]:
        """
    ...
    def _generate_reasoning(
        ...
        if domain != DomainType.GENERIC:
            parts.append(
                f"Detected {domain.value} domain (confidence: {domain_confidence:.1%})"
            )
        
        parts.append(f"Process complexity: {complexity.value}")
        
        if patterns:
            pattern_names = ", ".join([p[0] for p in patterns[:3]])
...
class ContextOptimizer:
    ...
    def optimize_context(
        ...
        reduction_ratio = max_tokens / context.token_count
