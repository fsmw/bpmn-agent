...
class LayoutInfo(BaseModel):
    ...
    waypoints: Optional[List[Waypoint]] = Field(
        None, description="Path waypoints for edges"
    )
...
class BPMNXMLGenerator:
    ...
    def __init__(self, enable_kb: bool = True):
        ...
        self.graph_id_to_bpmn_id: Dict[str, str] = {}  # Reverse mapping: graph node ID -> BPMN element ID
    ...
    def generate_xml(
        self, graph: ProcessGraph, process_name: Optional[str] = None
    ) -> str:
    ...
    def _create_sequence_flow(self, edge: GraphEdge, graph: ProcessGraph) -> None:
        ...
        if not source_elem or not target_elem:
            logger.warning(f"Cannot create flow: missing elements {edge.source_id} or {edge.target_id}")
            return
        
        flow_id = self._generate_id("SequenceFlow", edge.label or f"{edge.source_id}_to_{edge.target_id}")
        flow = SequenceFlow(
            id=flow_id,
            name=edge.label,
    ...
    def _record_id_mapping(
        self, graph_id: str, bpmn_id: str, element_type: str
    ) -> None:
