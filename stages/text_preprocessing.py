...
from knowledge import DomainClassifier, PatternRecognizer
...
class TextPreprocessor:
    ...
    def _validate_input(self, text: str) -> None:
        ...
        if len(text) < self.min_length:
            raise ValueError(
                f"Input too short: {len(text)} chars (minimum: {self.min_length})"
            )
        
        if len(text) > self.max_length:
            raise ValueError(
                f"Input too long: {len(text)} chars (maximum: {self.max_length})"
            )
        ...
        if len(text) > 0:
            ...
            if non_ascii_ratio > 0.8:
                raise ValueError(
                    f"Input has too many non-ASCII characters ({non_ascii_ratio:.1%})"
                )
