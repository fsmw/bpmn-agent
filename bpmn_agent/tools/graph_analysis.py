...
class GraphAnalyzer:
    ...
    def analyze_graph_structure(
        ...
        self.logger.info(f"Analysis complete: {len(result.anomalies)} anomalies, quality score: {result.quality_score:.2f}")
        ...
        for gateway in Gateways:
            ...
            if len(outgoing) > 1:
                ...
                if len(destinations) > 1:
                    ...
                    converge_points = self._find_convergence_points(graph, [e.target_id for e in outgoing])
        ...
        for edge in graph.edges:
            ...
            if target_node and not self._has_outgoing_edges(graph, edge.target_id):
                ...
                if incoming > 1:
                    ...
                    upstream_sources = [e.source_id for e in graph.edges if e.target_id == edge.target_id]
    ...
    def _calculate_basic_metrics(self, graph: ProcessGraph) -> Dict[str, Any]:
        ...
        return {
            ...
            ) / len(graph.nodes) if graph.nodes else 0,
            "is_connected": len(graph.edges) > 0 and any(
                self._get_node(graph, n.id).type.lower() == "start" 
                for n in graph.nodes
            ),
        }
    ...
    def _identify_structures(self, graph: ProcessGraph) -> List[GraphStructure]:
        ...
        if self._is_linear(graph):
            structures.append(GraphStructure(
    ...
    def _find_parallel_structures(self, graph: ProcessGraph) -> List[str]:
        ...
        and_gateways = [
            ...
            if n.type.lower() == "parallel_gateway" and 
            self._count_outgoing_edges(graph, n.id) > 1
        ]
    ...
    def _find_choice_structures(self, graph: ProcessGraph) -> List[str]:
        ...
        xor_gateways = [
            n for n in graph.nodes 
            if n.type.lower() in ("exclusive_gateway", "xor_gateway") and 
            self._count_outgoing_edges(graph, n.id) > 1
        ]
    ...
    def _generate_suggestions(self, result: GraphAnalysisResult) -> List[str]:
        ...
        if result.anomalies:
            ...
            if AnomalyType.CYCLE_DETECTED in anomaly_types:
                suggestions.append("Review cycles for potential infinite loops or missing exit conditions")
        
        # Suggestions for improvement
        if result.quality_score < 70:
            suggestions.append("Consider reviewing overall process structure for quality improvements")
        
        # Add implicit join suggestions
        implicit_joins = [s for s in result.structures if s.structure_type == StructureType.PARALLEL]
        if implicit_joins:
            suggestions.append("Consider adding explicit parallel gateways to improve process clarity")
