...
from bpmn_agent.models.extraction import ExtractionResultWithErrors, ExtractedEntity, ExtractedRelation
...
from bpmn_agent.tools.validation import ValidationResult, ValidationLevel
...
class ClarificationRequester:
    ...
    def request_clarification(
        ...
        if validation_result:
            questions.extend(self._generate_validation_questions(extraction_result, validation_result))
        ...
        # Sort by priority
        questions.sort(key=lambda q: {"critical": 4, "high": 3, "medium": 2, "low": 1}.get(q.priority, 1))
        
        self.logger.info(f"Generated {len(questions)} clarification questions")
        return questions
    
    def _generate_entity_questions(self, extraction_result: ExtractionResultWithErrors) -> List[ClarificationQuestion]:
        ...
        for entity in low_confidence[:3]:  # Limit to first 3
            questions.append(ClarificationQuestion(
                ...
                possible_answers=["Yes", "No, it's a different activity", "No, it's not part of the process"],
        ...
        generic_entities = [e for e in extraction_result.entities if e.type.lower() in ["activity", "task", "process", "system"]]
        for entity in generic_entities[:2]:  # Limit to first 2
            questions.append(ClarificationQuestion(
                question_type=ClarificationType.ENTITY_IDENTIFICATION,
                question=f"What specific type of activity is '{entity.name}'?",
                context=entity.source_context or "",
                possible_answers=["Human task", "Automated task", "Decision point", "External system action"],
                element_id=entity.identifier,
                element_type=entity.type,
                priority="high",
    ...
    def _generate_relation_questions(self, extraction_result: ExtractionResultWithErrors) -> List[ClarificationQuestion]:
        ...
        for entity_key in list(disconnected)[:3]:  # Limit to first 3
            ...
            questions.append(ClarificationQuestion(
                ...
                possible_answers=["Start of process", "End of process", "Connected to specific step", "Not part of main flow"],
    ...
    def _generate_actor_questions(self, extraction_result: ExtractionResultWithErrors) -> List[ClarificationQuestion]:
        ...
        actorless_tasks = [t for t in tasks if not any("actor" in r.type.lower() for r in extraction_result.relations if r.source_name == t.name)]
    ...
    def _generate_validation_questions(
        ...
        for issue in validation_result.issues:
            if issue.category.value == "extraction_quality" and issue.level == ValidationLevel.WARNING:
                if "No entities extracted" in issue.message:
                    questions.append(ClarificationQuestion(
                        ...
                        possible_answers=["Yes, it's a complete process", "Yes, but it's very simple", "No, it's just general text"],
                        priority="critical",
    ...
    def _generate_graph_questions(self, graph: ProcessGraph, extraction_result: ExtractionResultWithErrors) -> List[ClarificationQuestion]:
        ...
        for orphan_id in orphaned[:2]:  # Limit to first 2
            ...
            if node:
                questions.append(ClarificationQuestion(
                    ...
                    possible_answers=["Yes, add connection from start", "Yes, add connection from another step", "No, remove it"],
...
class ImprovementSuggester:
    ...
    def suggest_improvements(
        ...
        suggestions.sort(key=lambda s: (s.confidence, {"high": 3, "medium": 2, "low": 1}.get(s.impact, 2)), reverse=True)
    ...
    def _suggest_entity_improvements(self, extraction_result: ExtractionResultWithErrors) -> List[ImprovementSuggestion]:
        ...
        generic_entities = [e for e in extraction_result.entities if e.type.lower() in generic_types]
        if generic_entities:
            suggestions.append(ImprovementSuggestion(
                category=SuggestionCategory.ENTITY_EXTRACTION,
    ...
    def _suggest_validation_improvements(self, validation_result: ValidationResult) -> List[ImprovementSuggestion]:
...
class StageRerunner:
    ...
    async def rerun_stage(
        ...
        enhanced_context = self._build_enhanced_context(stage_name, improvements, clarifications, original_text)
        ...
        self.logger.info(f"Stage {stage_name} re-run completed, improvement indicated: {improvement_indicated}")
...
class RefinementOrchestrator:
    ...
    async def create_refinement_plan(
        ...
        self.logger.info(f"Created refinement plan {plan_id}: {len(plan.clarifications_needed)} clarifications, {len(plan.improvements_suggested)} improvements")
...
def suggest_improvements(
    ...
    return suggester.suggest_improvements(extraction_result, validation_result, graph, original_text)
...
async def rerun_stage(
    ...
    return await rerunner.rerun_stage(stage_name, original_result, improvements, clarifications, original_text)
...
async def create_refinement_plan(
    ...
    return await orchestrator.create_refinement_plan(extraction_result, validation_result, graph, original_text)