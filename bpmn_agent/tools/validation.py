...
class XMLValidator:
        ...
        try:
            ...
            if self.xsd_schema.validate(xml_doc):
                ...
                for error in error_log:
                    level = ValidationLevel.ERROR if error.level_name == "ERROR" else ValidationLevel.WARNING
                    result.issues.append(ValidationIssue(
                        level=level,
                        category=ValidationCategory.XML_COMPLIANCE,
            ...
            result.issues_by_level[ValidationLevel.ERROR] = result.issues_by_level.get(ValidationLevel.ERROR, 0) + 1
    ...
    def _validate_xml_structure(self, xml_content: str, result: ValidationResult) -> ValidationResult:
        ...
        try:
            ...
            if not start_events:
                ...
                result.issues_by_level[ValidationLevel.WARNING] = result.issues_by_level.get(ValidationLevel.WARNING, 0) + 1
...
class GraphValidator:
    ...
    def validate_graph_semantics(
        ...
        result.is_valid = result.issues_by_level.get(ValidationLevel.ERROR, 0) == 0 and result.issues_by_level.get(ValidationLevel.CRITICAL, 0) == 0
    ...
    def _validate_connectivity(self, graph: ProcessGraph, result: ValidationResult) -> None:
        ...
        if unreachable:
            ...
            result.issues_by_level[ValidationLevel.WARNING] = result.issues_by_level.get(ValidationLevel.WARNING, 0) + len(unreachable)
        ...
        for gateway in gateways:
            ...
            if outgoing_count > 1:
                ...
                if gateway.type.lower() in ["parallel_gateway", "and_gateway"]:
                    ...
                    matching_join = self._find_matching_gateway_join(graph, gateway.id, gateway.type)
    ...
    def _validate_swimlanes(self, graph: ProcessGraph, result: ValidationResult) -> None:
        ...
        if nodes_with_lanes and nodes_without_lanes:
            ...
            result.issues_by_level[ValidationLevel.INFO] = result.issues_by_level.get(ValidationLevel.INFO, 0) + min(5, len(nodes_without_lanes))
    ...
    def _find_matching_gateway_join(self, graph: ProcessGraph, split_gateway_id: str, gateway_type: str) -> Optional[str]:
...
class ExtractionValidator:
    ...
    def validate_extraction(self, extraction_result: ExtractionResultWithErrors) -> ValidationResult:
    ...
    def _validate_entities(self, extraction_result: ExtractionResultWithErrors, result: ValidationResult) -> None:
        ...
        if duplicate_count > 0:
            ...
            result.issues_by_level[ValidationLevel.INFO] = result.issues_by_level.get(ValidationLevel.INFO, 0) + 1
    
    def _validate_relations(self, extraction_result: ExtractionResultWithErrors, result: ValidationResult) -> None:
    ...
    def _validate_metadata(self, extraction_result: ExtractionResultWithErrors, result: ValidationResult) -> None:
        ...
        if metadata.extraction_duration_ms and metadata.extraction_duration_ms > 30000:  # 30 seconds
            result.issues.append(ValidationIssue(
                level=ValidationLevel.INFO,
                category=ValidationCategory.EXTRACTION_QUALITY,
