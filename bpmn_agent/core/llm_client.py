...
class LLMConfig(BaseModel):
    ...
    provider: LLMProviderType = Field(
        default=LLMProviderType.OLLAMA, description="LLM provider type"
    )
    ...
    api_key: Optional[str] = Field(
        default=None, description="API key for authentication (OpenAI, Azure, etc.)"
    )

    # Model settings
    model: str = Field(default="mistral", description="Model name or ID")
    model_type: Optional[ModelType] = Field(
        default=None, description="Specific model type for optimized configuration"
    )
    ...
    retry_backoff: float = Field(
        default=1.0, gt=0, description="Exponential backoff factor for retries"
    )

    # Context window settings
    context_window: int = Field(default=4096, gt=0, description="LLM context window size (tokens)")

    # Advanced settings
    system_prompt: Optional[str] = Field(
        default=None, description="Default system prompt for all requests"
    )

    # Azure-specific settings
    azure_deployment: Optional[str] = Field(
        default=None, description="Azure deployment name (for Azure OpenAI only)"
    )
    azure_api_version: str = Field(
        default="2024-02-15-preview", description="Azure API version (for Azure OpenAI only)"
    )
    ...
    def from_env(cls) -> "LLMConfig":
        ...
        if provider == "azure_openai":
            ...
            config_data["azure_api_version"] = os.getenv("AZURE_API_VERSION", "2024-02-15-preview")
...
class LLMResponse(BaseModel):
    ...
    finish_reason: Optional[str] = Field(
        default=None, description="Reason for completion: 'stop', 'length', 'error', etc."
    )
...
async def call_llm(
    messages: List[LLMMessage], config: Optional[LLMConfig] = None, **kwargs: Any
) -> LLMResponse:
    """
