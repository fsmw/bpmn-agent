...
class ErrorRecoveryEngine:
    ...
    async def execute_with_recovery(
        ...
        **kwargs,
        ...
        for attempt in range(self.retry_config.max_retries + 1):
            try:
                logger.debug(
                    f"{stage_name}: Attempt {attempt + 1}/{self.retry_config.max_retries + 1}"
                )
    ...
    def get_error_summary(self) -> dict:
        ...
        stages = {ctx.stage_name for ctx in self.error_history}
        error_types = {type(ctx.error).__name__ for ctx in self.error_history}
        ...
        return {
            ...
            "recovery_rate": (
                successful_recoveries / len(self.error_history) if self.error_history else 0.0
            ),
        }
...
class GracefulDegradationHandler:
    ...
    def create_empty_extraction_result():
        ...
        from bpmn_agent.models.extraction import ExtractionMetadata, ExtractionResultWithErrors
