        ...
        for node_id, node in graph.nodes.items():
            ...
            if node_type and 'gateway' in str(node_type).lower():
                ...
                if len(outgoing) > 1:
                    ...
                    split_target_nodes = set(e.target_id for e in outgoing)
...
class ProcessRefinementTools:
    ...
    def suggest_clarification_questions(
        ...
        if not lanes:
            questions.append(
                "Which roles or departments are involved in this process?"
            )
    ...
    def suggest_improvements(graph: GraphAnalysisResult) -> List[str]:
        ...
        if graph.cyclic_paths:
            suggestions.append(
                "The process contains loops. Verify these are intentional."
            )
