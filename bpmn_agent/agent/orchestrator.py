...
class BPMNAgent:
    ...
    async def _stage2_extract_entities(
        ...
        with self.observability_hooks.track_stage(
            stage_name, 
            attributes={"use_kb": use_kb, "domain": domain}
        ) as stage_metrics:
            try:
                # Extract entities
                ...
                stage_metrics.add_attribute("avg_confidence", self.state.metrics.avg_entity_confidence)
    ...
    async def _stage3_resolve_entities(
        ...
        with self.observability_hooks.track_stage(stage_name) as stage_metrics:
            try:
                ...
                # Record stage metrics
                stage_metrics.success = True
                stage_metrics.add_attribute("coreferences_resolved", self.state.metrics.coreferences_resolved)
                stage_metrics.add_attribute("actors_consolidated", self.state.metrics.actors_consolidated)
    ...
    async def _stage5_generate_xml(
        ...
        with self.observability_hooks.track_stage(
            stage_name,
            attributes={"process_name": process_name, "domain": domain}
        ) as stage_metrics:
            try:
                # Generate XML
                ...
                # Record stage metrics
                stage_metrics.success = True
                stage_metrics.add_attribute("xml_size_bytes", self.state.metrics.output_xml_size_bytes)
                stage_metrics.add_attribute("element_count", self.state.metrics.output_element_count)
