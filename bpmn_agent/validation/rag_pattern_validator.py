...
class RAGPatternValidator:
    ...
    def __init__(
        ...
        try:
            ...
            if pattern_bridge is None:
                from bpmn_agent.knowledge.pattern_matching_bridge import (
                    AdvancedPatternMatchingBridge,
                )
    ...
    def validate_pattern_compliance(
        ...
        if not self.enabled or not patterns_applied:
            return RAGValidationResult(
                findings=[], overall_compliance_score=1.0, patterns_validated=0, patterns_passed=0
            )
        ...
        for pattern_id in patterns_applied:
            try:
                ...
                if not pattern:
                    self.logger.warning(
                        f"Pattern {pattern_id} not found in KB, skipping validation"
                    )
                    continue

                finding = self._validate_single_pattern(xml_content, pattern, graph)
    ...
    def _validate_single_pattern(
        self, xml_content: str, pattern: BPMNPattern, graph: Optional[ProcessGraph]
        ...
        structure_score = self._validate_pattern_structure(
            xml_content, pattern, issues, suggestions
        )
        ...
        overall_score = structure_score * 0.4 + element_score * 0.4 + relation_score * 0.2
    ...
    def _validate_pattern_structure(
        self, xml_content: str, pattern: BPMNPattern, issues: List[str], suggestions: List[str]
    ) -> float:
        ...
        if score < 0.5:
            ...
            suggestions.append(
                f"Increase presence of pattern-specific elements: {', '.join(pattern_keywords[:3])}"
            )
    ...
    def _validate_pattern_elements(
        ...
        if pattern.graph_structure and pattern.graph_structure.nodes:
            ...
            if expected_elements:
                ...
                if score < 0.7:
                    issues.append(
                        f"Pattern '{pattern.name}' elements not fully present ({score:.2f} coverage)"
                    )
                    suggestions.append(
                        f"Ensure all pattern elements are generated: {', '.join(list(expected_elements)[:3])}"
                    )
    ...
    def _validate_pattern_relations(
        ...
        for edge in graph.edges:
            source_id = (
                edge.source_id if hasattr(edge, "source_id") else getattr(edge, "source", None)
            )
            target_id = (
                edge.target_id if hasattr(edge, "target_id") else getattr(edge, "target", None)
            )
        ...
        if score < 0.7:
            issues.append(
                f"Pattern '{pattern.name}' relations not fully matched ({score:.2f} coverage)"
            )
            suggestions.append(
                f"Ensure pattern relations are correctly implemented: {', '.join(list(pattern_edges)[:2])}"
            )
