...
class Phase4TestSuite:
    ...
    def test_enhanced_xsd_validation_all_examples(self) -> Dict:
        ...
        for test_case in self.test_cases:
            ...
            try:
                ...
                logger.info(
                    f"  ✅ Enhanced XSD validation completed: valid={validation_result.is_valid}, score={validation_result.quality_score:.2f}"
                )
            ...
            try:
                ...
                if analysis_result:
                    ...
                    test_result = {
                        ...
                        "structure_types": [
                            s.structure_type.value for s in graph_analysis.structures
                        ],
                    ...
                    logger.info(
                        f"✅ Graph analysis completed: nodes={graph_analysis.total_nodes}, quality={graph_analysis.quality_score:.2f}"
                    )
                else:
                    results["successful_analyses"] += 1
        ...
        for test_case in self.test_cases:
            ...
            try:
                ...
                improvements = self.improvement_suggester.suggest_improvements(
                    extraction_result, graph=graph, original_text=test_case.description or ""
                )
                ...
                for suggestion in improvements:
                    ...
                    results["suggestions_by_category"][category] = (
                        results["suggestions_by_category"].get(category, 0) + 1
                    )
                ...
                logger.info(
                    f"✅ Improvement suggestions generated: {len(improvements)} suggestions for {test_case.file_name}"
                )
        ...
        for test_case in self.test_cases:
            ...
            try:
                ...
                test_result = {
                    ...
                    "is_correct": (
                        test_case.domain == domain_result.domain.value if domain_result else False
                    ),
                ...
                logger.info(
                    f"✅ Domain classification: {test_result['detected_domain']} (confidence: {test_result['confidence']:.2f})"
                )
    ...
    def test_comprehensive_validation_workflow(self) -> Dict:
        ...
        for test_case in self.test_cases:
            ...
            try:
                ...
                try:
                    domain_result = self.domain_classifier.classify_domain(test_case.xml_content)
                    workflow_result["detected_domain"] = (
                        domain_result.domain.value if domain_result else "unknown"
                    )
                ...
                logger.info(
                    f"✅ Comprehensive workflow: success={workflow_result['workflow_success']}, score={workflow_result.get('overall_score', 0):.1f}"
                )
        ...
        success_rate = (
            results["successful_workflows"] / results["total_workflows"]
            if results["total_workflows"] > 0
            else 0
        )
        avg_score = (
            results["average_compliance_score"] / results["successful_workflows"]
            if results["successful_workflows"] > 0
            else 0
        )
    ...
    def _create_graph_from_xml(self, xml_content: str) -> Optional[ProcessGraph]:
        ...
        try:
            ...
            for elem in xml_doc.iter():
                if "sequenceFlow" in elem.tag.lower() or (
                    elem.get("sourceRef") and elem.get("targetRef")
                ):
    ...
    def _log_test_category_summary(self, category: str, results: Dict) -> None:
        ...
        logger.info("  Detailed results available in test_results")
