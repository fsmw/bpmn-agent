...
from bpmn_agent.tools.validation import ValidationLevel
                ...
                try:
                    with open(self.LOCAL_XSD_PATH, "rb") as f:
                        ...
                        try:
                            ...
                            self.logger.info(
                                f"BPMN 2.0 XSD schema loaded from local file: {self.LOCAL_XSD_PATH}"
                            )
                            return
                            ...
                            self.logger.warning(
                                f"XSD schema loaded but external references may be missing: {schema_error}"
                            )
...
<xsd:element name="relationship"/>
    ...
    def validate_xml_against_xsd(
        ...
        result = XSDValidationResult(is_valid=True, total_errors=0, total_warnings=0)
        ...
        try:
            ...
            if self.enable_kb_patterns and (graph or extraction_result):
                self._validate_semantic_compliance(
                    xml_content, result, graph, extraction_result, domain, patterns_applied
                )
        ...
        result.total_errors = sum(
            result.errors_by_level[level]
            for level in result.errors_by_level
        ...
        for error in error_log:
            ...
            if xpath:
                try:
                    ...
                    element_name = (
                        xpath.split("/")[-1].split("[")[0] if "[" in xpath else xpath.split("/")[-1]
                    )
                    # Try to extract ID
    ...
    def _categorize_error(self, error) -> ValidationErrorCategory:
        ...
        elif "attribute" in message_lower and (
            "invalid" in message_lower or "illegal" in message_lower
        ):
    ...
    def _validate_xml_structure(self, xml_doc: ET.Element, result: XSDValidationResult) -> None:
        ...
        if result.errors:
            for error in result.errors:
                ...
                result.errors_by_category[error.category] = (
                    result.errors_by_category.get(error.category, 0) + 1
                )
    ...
    def _validate_domain_semantics(
        self, xml_content: str, result: XSDValidationResult, domain: str
    ) -> None:
    ...
    def _validate_hr_semantics(self, xml_content: str, result: XSDValidationResult) -> None:
        ...
        for element_name in hr_elements:
            found_elements.update(
                [elem.tag for elem in xml_doc.iter() if element_name.lower() in elem.tag.lower()]
            )

        missing_elements = [
            elem for elem in hr_elements if elem not in found_elements and elem not in ["task"]
        ]
        if missing_elements:
            result.errors.append(
                XSDValidationError(
            ...
            result.errors_by_level[XSDValidationErrorLevel.INFO] = (
                result.errors_by_level.get(XSDValidationErrorLevel.INFO, 0) + 1
            )
            result.errors_by_category[ValidationErrorCategory.DOMAIN_SPECIFIC] = (
                result.errors_by_category.get(ValidationErrorCategory.DOMAIN_SPECIFIC, 0) + 1
            )
    ...
    def _validate_finance_semantics(self, xml_content: str, result: XSDValidationResult) -> None:
        ...
        if "approval" not in found_elements:
            ...
            result.errors_by_level[XSDValidationErrorLevel.WARNING] = (
                result.errors_by_level.get(XSDValidationErrorLevel.WARNING, 0) + 1
            )
            result.errors_by_category[ValidationErrorCategory.DOMAIN_SPECIFIC] = (
                result.errors_by_category.get(ValidationErrorCategory.DOMAIN_SPECIFIC, 0) + 1
            )
    ...
    def _validate_pattern_application(
        self, xml_content: str, result: XSDValidationResult, patterns_applied: List[str]
    ) -> None:
        ...
        for pattern in patterns_applied:
            ...
            if "approval_sequence" in pattern.lower():
                if "approval" not in xml_content.lower():
                    ...
                    result.errors_by_category[ValidationErrorCategory.PATTERN_VIOLATION] = (
                        result.errors_by_category.get(ValidationErrorCategory.PATTERN_VIOLATION, 0)
                        + 1
                    )
    ...
    def _calculate_quality_metrics(
        ...
        if result.total_errors + result.total_warnings > 0:
            metrics["xsd_compliance_rate"] = 100.0 - (
                (result.total_errors * 10) / (result.total_errors + result.total_warnings)
            )
        ...
        result.quality_score = (
            quality_percentage / 100.0 if quality_percentage > 1.0 else quality_percentage
        )
    ...
    def _validate_pattern_compliance(
        self, result: XSDValidationResult, patterns_applied: List[str]
    ) -> None:
        ...
        if pattern_issues:
            result.errors_by_category[ValidationErrorCategory.PATTERN_VIOLATION] = len(
                pattern_issues
            )
            ...
            pattern_score = (
                ...
                if pattern_compliance_rate > 1.0
                else pattern_compliance_rate
            )
            result.quality_score = result.quality_score * 0.8 + pattern_score * 0.2
