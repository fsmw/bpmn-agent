...
class RAGFeedbackLoop:
    ...
    def _update_pattern_metrics(
        self, pattern_id: str, validation_result: XSDValidationResult
    ) -> None:
        """Update effectiveness metrics for a pattern."""
        if pattern_id not in self.pattern_metrics:
            self.pattern_metrics[pattern_id] = PatternEffectivenessMetrics(pattern_id=pattern_id)
    ...
    def get_pattern_recommendations(
        self, domain: DomainType, max_recommendations: int = 5
    ) -> List[str]:
        """
        ...
        effective_patterns = [
            (pid, metrics)
            for pid, metrics in self.pattern_metrics.items()
        ...
        effective_patterns.sort(
            key=lambda x: x[1].times_successful / max(x[1].times_applied, 1), reverse=True
        )
    ...
    def get_feedback_summary(self) -> Dict[str, Any]:
        ...
        top_patterns = sorted(
            self.pattern_metrics.items(), key=lambda x: x[1].average_quality_score, reverse=True
        )[:5]
