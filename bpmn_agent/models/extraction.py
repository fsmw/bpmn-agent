...
class EntityAttribute(BaseModel):
    ...
    value_type: str = Field(
        default="string", description="Value type: string, number, boolean, list, etc."
    )
    confidence: ConfidenceLevel = Field(
        default=ConfidenceLevel.MEDIUM, description="Confidence in this attribute"
    )
    source_text: Optional[str] = Field(
        None, description="Source text snippet where this was extracted"
    )
...
class ExtractedEntity(BaseModel):
    ...
    confidence: ConfidenceLevel = Field(
        default=ConfidenceLevel.MEDIUM, description="Confidence in entity extraction"
    )

    # Source information
    source_text: Optional[str] = Field(None, description="Original text snippet from input")
    character_offsets: Optional[tuple[int, int]] = Field(
        None, description="Start and end character positions in input"
    )
    ...
    is_implicit: bool = Field(
        False, description="Whether entity was inferred vs. explicitly mentioned"
    )
    is_uncertain: bool = Field(False, description="Whether extraction is uncertain")
    alternative_names: List[str] = Field(
        default_factory=list, description="Alternate names/aliases for this entity"
    )
...
class ExtractedRelation(BaseModel):
    ...
    confidence: ConfidenceLevel = Field(
        default=ConfidenceLevel.MEDIUM, description="Confidence in relationship extraction"
    )

    # Source information
    source_text: Optional[str] = Field(
        None, description="Original text snippet describing this relationship"
    )
    ...
    # Metadata
    is_implicit: bool = Field(
        False, description="Whether relationship was inferred vs. explicitly stated"
    )
    is_conditional: bool = Field(False, description="Whether this relationship is conditional")
    condition_expression: Optional[str] = Field(None, description="Guard condition if conditional")
...
class CoReferenceGroup(BaseModel):
    ...
    canonical_id: str = Field(..., description="ID of canonical entity representation")
    canonical_form: str = Field(..., description="Preferred/canonical form of the entity name")
    mentions: List[str] = Field(
        default_factory=list,
        description="IDs of extracted entities that refer to this canonical form",
    )
    mention_texts: List[str] = Field(
        default_factory=list, description="Original text forms of mentions"
    )
    confidence: ConfidenceLevel = Field(
        default=ConfidenceLevel.MEDIUM, description="Confidence in co-reference resolution"
    )
...
class ExtractionResult(BaseModel):
    ...
    co_references: List[CoReferenceGroup] = Field(
        default_factory=list, description="Co-reference groups (entity disambiguation)"
    )
...
class ExtractionError(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        None, description="Contextual information about the error"
    )
    recoverable: bool = Field(True, description="Whether pipeline can continue after this error")
