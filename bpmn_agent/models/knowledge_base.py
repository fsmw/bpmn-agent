...
class GraphStructure(BaseModel):
    ...
    nodes: List[str] = Field(..., description="Node identifiers (gateways, activities, events)")
...
class BPMNPattern(BaseModel):
    ...
    domain: DomainType = Field(
        default=DomainType.GENERIC, description="Primary domain for this pattern"
    )
    category: PatternCategory = Field(..., description="Pattern type/category")
    complexity: ComplexityLevel = Field(
        default=ComplexityLevel.MODERATE, description="Complexity level of the pattern"
    )
    ...
    examples: List[str] = Field(
        default_factory=list, description="Text examples where this pattern appears"
    )
    validation_rules: List[str] = Field(
        default_factory=list, description="Rules to validate pattern compliance"
    )
    anti_patterns: List[str] = Field(
        default_factory=list, description="Common mistakes or anti-patterns to avoid"
    )

    # Searchability and Metadata
    tags: Set[str] = Field(default_factory=set, description="Searchable keywords")
    related_patterns: List[str] = Field(default_factory=list, description="IDs of related patterns")

    # Quality and Tracking
    confidence: float = Field(
        default=0.9, ge=0.0, le=1.0, description="Quality/confidence score (0-1)"
    )
    usage_count: int = Field(default=0, description="Number of times this pattern has been used")
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Pattern creation timestamp"
    )
...
class DomainExample(BaseModel):
    ...
    # Classification
    domain: DomainType = Field(default=DomainType.GENERIC, description="Domain category")
    complexity: ComplexityLevel = Field(
        default=ComplexityLevel.MODERATE, description="Complexity level"
    )

    # Pattern Information
    patterns_used: List[str] = Field(
        default_factory=list, description="IDs of patterns present in this example"
    )
    ...
    relations_expected: Dict[str, Any] = Field(
        default_factory=dict, description="Expected relations in the process"
    )

    # Quality Metrics
    difficulty: str = Field(default="medium", description="Example difficulty: easy, medium, hard")
    validation_score: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Quality score for this example (0-1)"
    )

    # Metadata
    bpmn_structure: Optional[Dict[str, Any]] = Field(
        None, description="Expected BPMN XML structure (optional)"
    )
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Creation timestamp"
    )
    usage_count: int = Field(default=0, description="Number of times used in training/inference")
...
class ContextPackage(BaseModel):
    ...
    selected_patterns: List[BPMNPattern] = Field(
        default_factory=list, description="Top N relevant patterns"
    )
    selected_examples: List[DomainExample] = Field(
        default_factory=list, description="3-5 few-shot examples"
    )
    domain_terms: Dict[str, List[str]] = Field(
        default_factory=dict, description="Domain-specific terminology by category"
    )
    validation_rules: List[str] = Field(
        default_factory=list, description="Relevant validation rules for the domain"
    )

    # Metadata
    detected_domain: DomainType = Field(
        default=DomainType.GENERIC, description="Detected primary domain"
    )
    detected_complexity: ComplexityLevel = Field(
        default=ComplexityLevel.MODERATE, description="Detected complexity level"
    )
    recognized_patterns: List[str] = Field(
        default_factory=list, description="IDs of patterns recognized in input text"
    )

    # Quality and Optimization Metrics
    token_count: int = Field(default=0, description="Estimated token count for this context")
    max_tokens: int = Field(default=4000, description="Maximum tokens available for context")
    confidence: float = Field(default=0.8, ge=0.0, le=1.0, description="Overall confidence (0-1)")

    # Optimization Details
    reasoning: str = Field(
        default="", description="Explanation of why these elements were selected"
    )
    optimization_level: str = Field(
        default="balanced", description="Optimization strategy: minimal, balanced, comprehensive"
    )
    created_at: datetime = Field(
...
class KnowledgeBaseMetadata(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="KB creation date"
    )
    last_updated: datetime = Field(
    ...
    domains_covered: Set[DomainType] = Field(
        default_factory=set, description="Domains with patterns"
    )
    description: str = Field(default="", description="KB description")
...
class KnowledgeBase(BaseModel):
    ...
    patterns: Dict[str, BPMNPattern] = Field(
        default_factory=dict, description="All patterns indexed by ID"
    )
    examples: Dict[str, DomainExample] = Field(
        default_factory=dict, description="All examples indexed by ID"
    )

    # Domain Vocabularies
    domain_vocabularies: Dict[DomainType, List[str]] = Field(
        default_factory=dict, description="Domain-specific terminology"
    )
    domain_keywords: Dict[DomainType, Dict[str, float]] = Field(
        default_factory=dict, description="Domain keywords with relevance scores"
    )

    # Validation and Quality
    anti_patterns: List[str] = Field(
        default_factory=list, description="Known anti-patterns to flag"
    )
    validation_templates: Dict[str, List[str]] = Field(
        default_factory=dict, description="Validation rule templates by pattern type"
    )

    # Metadata
    metadata: KnowledgeBaseMetadata = Field(
        default_factory=KnowledgeBaseMetadata, description="KB metadata"
    )

    # Vector Store Reference
    vector_store_ready: bool = Field(
        default=False, description="Whether vector embeddings have been built"
    )
    ...
    def get_patterns_by_category(self, category: PatternCategory) -> Dict[str, BPMNPattern]:
        ...
        return {
            pid: pattern for pid, pattern in self.patterns.items() if pattern.category == category
        }
