...
class AdvancedPatternMatchingBridge:
    ...
    def find_patterns_for_process(
        self, process_description: str, domain: Optional[DomainType] = None, max_patterns: int = 5
    ) -> PatternRecommendation:
        """
        ...
        return self.matcher.get_pattern_recommendations(
            context_text=process_description, domain=domain, max_patterns=max_patterns
        )

    def match_activity_to_patterns(
        self, activity_name: str, activity_description: str, domain: Optional[DomainType] = None
    ) -> List[MatchResult]:
        """
        ...
        return self.matcher.composite_search(query=query, domain=domain, max_results=5)

    def find_similar_patterns_for_pattern(
        self, pattern_id: str, max_similar: int = 3
    ) -> List[BPMNPattern]:
        """
    ...
    def validate_extracted_activities(
        self, activities: List[str], domain: Optional[DomainType] = None
    ...
    def enrich_pattern_context(self, pattern_id: str) -> Dict:
        """
    ...
    def suggest_patterns_by_domain(
        ...
        if complexity:
            patterns = {pid: p for pid, p in patterns.items() if p.complexity == complexity}

        # Sort by usage and confidence
        sorted_patterns = sorted(
            patterns.items(), key=lambda x: (x[1].usage_count, x[1].confidence), reverse=True
        )

        # Return top patterns
        result = []
        for _pattern_id, pattern in sorted_patterns[:max_patterns]:
            result.append(
                {
                    "id": pattern.id,
                    "name": pattern.name,
    ...
    def get_pattern_statistics(self) -> Dict:
        ...
        stats["metadata"] = {
            ...
            "created_at": (
                self.kb.metadata.created_at.isoformat() if self.kb.metadata.created_at else None
            ),
            "last_updated": (
                self.kb.metadata.last_updated.isoformat() if self.kb.metadata.last_updated else None
            ),
        }
