...
class MatchResult:
    ...
    relevance_factors: Dict[str, float] = field(default_factory=dict)  # Breakdown of scoring factors
...
class AdvancedPatternMatcher:
    ...
    def match_by_keywords(
        ...
        for pattern_id in pattern_ids:
            ...
            if score >= threshold:
                relevance_factors = {
                    "exact_matches": len(exact_matches) / len(query_keywords) if query_keywords else 0,
        ...
        for pattern_id in pattern_ids:
            ...
            if avg_score >= threshold:
                match_result = MatchResult(
                    ...
                    match_type="semantic",
                    confidence=pattern.confidence,
                    reasoning=f"Semantic similarity to pattern description",
                    relevance_factors={
    ...
    def composite_search(
        ...
        # Return top results
        results = []
        for pattern_id, (match, score) in sorted_results[:max_results]:
    ...
    def get_pattern_recommendations(
        ...
        matches = self.composite_search(
            context_text,
            domain=domain,
            max_results=max_patterns * 2
        )
        ...
        detected_complexity = max(
            set(complexities),
            key=complexities.count
        ) if complexities else ComplexityLevel.MODERATE
        ...
        reasoning += f"Pattern types: {', '.join(set(c.value for c in categories))}"
        
        # Average confidence
        avg_confidence = sum(m.match_score for m in matches[:max_patterns]) / len(matches[:max_patterns])
