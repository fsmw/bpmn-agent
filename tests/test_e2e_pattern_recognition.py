    ...
    def test_all_domain_samples_present(self):
        ...
        for domain, samples in domains.items():
            ...
            for name, description in samples.items():
                assert (
                    len(description) > 100
                ), f"{domain}:{name} should have substantial description"
...
class TestPatternLibraryLoading:
    ...
    def test_healthcare_patterns_loaded(self):
        ...
        with open(
            "/home/fsmw/dev/bpmn/src/bpmn-agent/bpmn_agent/knowledge/patterns/healthcare_patterns.json"
        ) as f:
    ...
    def test_manufacturing_patterns_loaded(self):
        ...
        with open(
            "/home/fsmw/dev/bpmn/src/bpmn-agent/bpmn_agent/knowledge/patterns/manufacturing_patterns.json"
        ) as f:
            patterns = json.load(f)
    ...
    def test_all_pattern_files_valid_json(self):
        ...
        pattern_files = glob.glob(
            "/home/fsmw/dev/bpmn/src/bpmn-agent/bpmn_agent/knowledge/patterns/*_patterns.json"
        )
...
class TestPatternCoverageAndQuality:
    ...
    def test_pattern_domains_have_minimum_patterns(self):
        ...
        for domain, filepath in domains.items():
            ...
            assert (
                len(patterns) >= min_count
            ), f"{domain} has {len(patterns)} patterns, expected at least {min_count}"
    ...
    def test_patterns_have_meaningful_keywords(self):
        ...
        for pattern_file in pattern_files:
            ...
            for pattern_id, pattern in patterns.items():
                ...
                assert all(
                    isinstance(k, str) and len(k) > 0 for k in keywords
                ), f"{pattern_id} has invalid keywords"
