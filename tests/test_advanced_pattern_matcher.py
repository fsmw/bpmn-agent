...
class TestCategoryMatching:
    ...
    def test_match_by_category_with_domain(self, matcher):
        ...
        results = matcher.match_by_category(
            PatternCategory.EXCLUSIVE_CHOICE, domain=DomainType.HEALTHCARE
        )
        assert len(results) > 0
        for r in results:
            assert r.pattern.category == PatternCategory.EXCLUSIVE_CHOICE
            assert (
                r.pattern.domain == DomainType.HEALTHCARE or r.pattern.domain == DomainType.GENERIC
            )
...
class TestComplexityMatching:
    ...
    def test_match_by_complexity_with_domain(self, matcher):
        ...
        assert all(
            r.pattern.complexity == ComplexityLevel.MODERATE
            and (r.pattern.domain == DomainType.FINANCE or r.pattern.domain == DomainType.GENERIC)
            for r in results
        )
...
class TestSemanticSimilarity:
    ...
    def test_semantic_similarity_with_domain(self, matcher):
        ...
        results = matcher.semantic_similarity(
            "financial transaction approval", domain=DomainType.FINANCE
        )
        assert len(results) > 0
...
class TestPatternRecommendation:
    ...
    def test_get_recommendations_with_domain(self, matcher):
        ...
        rec = matcher.get_pattern_recommendations(
            "process patient consent before treatment", domain=DomainType.HEALTHCARE
        )
...
class TestPatternValidation:
    ...
    def test_validate_non_matching_pattern(self, matcher):
        ...
        is_valid, confidence, issues = matcher.validate_pattern_match(
            matcher.kb.get_pattern("sequential_001"), "asdf qwerty zxcv random words"
        )
    ...
    def test_validation_details(self, matcher):
        ...
        is_valid, confidence, issues = matcher.validate_pattern_match(
            matcher.kb.get_pattern("it_incident_001"), "Short text"
        )
        assert isinstance(issues, list)
