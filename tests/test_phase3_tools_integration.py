...
class TestToolSuiteIntegration:
    ...
    def sample_extraction_result(self):
        ...
        return ExtractionResultWithErrors(
            entities=entities, relations=relations, co_references=[], metadata=metadata, errors=[]
        )
    ...
    def sample_graph(self):
        ...
        nodes = [
            ...
            GraphNode(id="task_1", type="task", label="Submit Request", bpmn_type="bpmn:Task"),
            GraphNode(
                id="gateway_1",
            ...
            GraphNode(
                id="isolated_task", type="task", label="Orphaned Task", bpmn_type="bpmn:Task"
            ),
        ...
        edges = [
            ...
            GraphEdge(source_id="task_1", target_id="gateway_1", type="sequence_flow", label=""),
...
<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
    ...
    def test_graph_analysis_integration(self, sample_graph, sample_extraction_result):
        ...
        assert (
            AnomalyType.ISOLATED_NODE in anomaly_types or AnomalyType.ORPHANED_NODE in anomaly_types
        )
    ...
    def test_validation_integration(self, sample_graph, sample_extraction_result, sample_xml):
        ...
        graph_result = graph_validator.validate_graph_semantics(
            sample_graph, sample_extraction_result
        )
    ...
    def test_tools_with_orchestrator_mock(self, sample_extraction_result, sample_graph):
        ...
        with pytest.MonkeyPatch().context() as m:
            ...
            m.setattr(
                "bpmn_agent.core.llm_client.LLMClientFactory.create", Mock(return_value=mock_client)
            )
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_tool_workflow(
        self, sample_extraction_result, sample_graph, sample_xml
    ):
        ...
        improvements = suggest_improvements(
            sample_extraction_result, graph_validation, sample_graph
        )
        assert isinstance(improvements, list)

        # Step 5: Request clarifications for ambiguities
        clarifications = request_clarification(
            sample_extraction_result, graph_validation, sample_graph
        )
