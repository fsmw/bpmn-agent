...
def create_graph(name: str) -> ProcessGraph:
    ...
    return ProcessGraph(
        id=f"{name.lower()}-{id(object())}", name=name, created_timestamp=datetime.now().isoformat()
    )
...
def graph_with_actors():
    ...
    actor2 = GraphNode(id="actor_approver", type=NodeType.ACTOR, label="Approver", bpmn_type="lane")
    ...
    graph.edges = [
        ...
        # Swimlane edges
        GraphEdge(
            id="swimlane1", source_id="task1", target_id="actor_finance", type=EdgeType.SWIMLANE
        ),
        GraphEdge(
            id="swimlane2", source_id="task2", target_id="actor_approver", type=EdgeType.SWIMLANE
        ),
    ]
...
def test_generate_xml_with_condition_expressions(xml_generator, complex_graph_with_gateways):
    ...
    flows = root.findall(".//bpmn:sequenceFlow", ns)
    condition_flows = [f for f in flows if f.find("bpmn:conditionExpression", ns) is not None]
